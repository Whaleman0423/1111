...
# 確認 csv 欄位有按照順序：
# question_id、question_content、question_cn_content、A、B、C、D、true_answer、CN_A, CN_B, CN_C, CN_D, detail_explain, expert_comment, question_domain_attribute, question_architecture_attribute, question_servcie_attribute
# db = firestore.Client()
db = firestore.Client.from_service_account_json("./cloud-master-3-29-cfb7e9371055.json", project='cloud-master-3-29')

# 開啟 CSV 檔案
with open('cloud_ass_engineer_v3 - output.csv', newline='', encoding="utf-8") as csvfile:
  ...
  for row in rows:
    ...
    # d["question_attribute"] = [row[14], row[15], row[16]] # 標籤
    d["question_attribute"] = [row[14]] # 標籤

# 儲存 json
    with open("CloudMasterExamQuestionGcpAce.json", 'a', encoding="utf-8") as fout:
      json.dump(d, fout, ensure_ascii=False)
      fout.write("\n")
# 上傳 firestore
        
    db.collection(u'CloudMasterExamQuestionGcpAce').document(str(i)).set(d)
    i += 1
    list_.append(d)

# 都上傳完畢後，查看目前 Firestore 資料集 CloudMasterExamQuestionGcpAce 有幾筆檔案
exams_qa_ref = db.collection(u'CloudMasterExamQuestionGcpAce')
docs = exams_qa_ref.stream()
n = 0
for doc in docs:
  n +=1
print('總題數: ', n, '題')
db.collection(u'CloudMasterExam').document(u'CloudMasterExamQuestionGcpAce').set({
  u'exam_name': u'CloudMasterExamQuestionGcpAce',
  u'deduct_point_amount': 1,
  u'number_of_questions': n,
  u'stat_name': u'CloudMasterExamQuestionGcpAceUserStat'
})
...
df.to_csv("CloudMasterExamQuestionGcpAce.csv")