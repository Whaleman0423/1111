...
def hello_gcs(event, context):
    ...
    方法邏輯描述：
        ...
        4. 清洗完後, 檢查 bigquery 指定 project 中的指定資料集是否有 main 資料表
        5. 沒有的話, 用 /tmp/output.json 建一個 main 資料表
        6. 有的話, 用 /tmp/output.json 建一個 new_add 資料表,將 new_add merge 到 main 資料表, 完成更新, 最後刪除 new_add 資料表
        7. 最後, 將function 內的暫存, /tmp/output.json 刪除掉
        8. 函式結束
...
# 目前 /tmp/test/ 裡面有 eml 檔案
    ...
    # if context.event_type == "google.storage.object.finalize" and event['name'].endswith('.json'):  # 若事件是 finalize, 且檔名為 .json 結尾, 就上傳 json 到 bigquery #### 不要了 直接來
# 先檢查 主要表格 有沒有存在, 存在的話要 merge；不存在的話，就建立一個 主要表格
        # print('收到 json 檔案')
        client = bigquery.Client()
        table_id = 'polar-decorator-330410.merge_test.main'   # 主要表格 => main；merge_test 是資料集
        try:            
            client.get_table(table_id)  # Make an API request.
            print("Table {} already exists.".format(table_id))   # 主要表格 存在
            
            # 主要表格存在的話，就執行以下程式碼, 1. 建立添加表格 2.merge 主要表格和添加表格 3.刪除添加表格
            # 先將 Cloud Storage 的 output.json 取到 /tmp 暫存
            # bucket_name="my_test_bucket0101"  # bucket 的名字
            # source_blob_name=event['name']  # 上傳檔案的名稱
            # bucket = storage_client.bucket(bucket_name)
            # blob = bucket.blob(source_blob_name)  # blob 應該是 json 檔案
            # print("blob上傳物件：", blob)
            # blob.download_to_filename('/tmp/output.json')  # 將 blob 下載到 function 的暫存空間
            # print('download output_json success')
            ...
            merge_job = bigquery_client.query(
            """
            MERGE
            `polar-decorator-330410.merge_test.main` A
            USING
            `polar-decorator-330410.merge_test.new_add` B
            ON
            A.id = B.id
            WHEN MATCHED THEN UPDATE SET A.name = B.name, A.gender=B.gender, A.work_experiences=B.work_experiences, A.work_experience_list=B.work_experience_list, A.computer_expertises=B.computer_expertises, A.wanted_job_locations=B.wanted_job_locations, A.wanted_job_types=B.wanted_job_types, A.wanted_job_titles=B.wanted_job_titles, A.edu_department=B.edu_department, A.address=B.address, A.edu_school=B.edu_school, A.edu_level=B.edu_level, A.id=B.id, A.cell_phone=B.cell_phone, A.age=B.age, A.working_months=B.working_months, A.email=B.email, A.languages=B.languages, A.edu_status=B.edu_status, A.platform=B.platform
            WHEN NOT MATCHED
            THEN
            INSERT
            (id,
                ...
                platform)
            VALUES
            (id, work_experiences, name, gender,work_experience_list,computer_expertises,wanted_job_locations,wanted_job_types,wanted_job_titles,edu_department,address,edu_school,edu_level,cell_phone,age,working_months,email,languages,edu_status,platform)"""
                )

            results = merge_job.result()  # Waits for job to complete.
            print("已merge好了")
            
            # 刪除 添加表格
            table_id = 'polar-decorator-330410.merge_test.new_add'
        ...
        except NotFound:  # 如果沒有找到 main 表格, 就建一個 main表格
